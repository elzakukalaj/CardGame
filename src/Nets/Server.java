...
import Logic.Logic;
import Logic.ProcessorConnection;
...
public class Server implements Runnable, CallBack{
    ...
    public Server(int listeningPort) throws IOException {
        ...
        this.tableCards = new CardNotes[4];
        ...
        while (true) {
            if(this.listinegFlag) {
                ...
                try {
                    ...
                    NodeConnection player = new NodeConnection(client,this,new ProcessorConnection());
                    player.shakeHands();
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void callBack(String Data , Object Caller) {
        ...
        if(Flag.equals("C_RNM")) {
            ...
            if(connectedNodes.size() == 4) {
                ...
                Logic.sleep(2000);
                ArrayList<ArrayList<CardNotes>> Cards = Logic.generateCardOfPlayers();
                for(int i =0 ;i<4; i++) {
                    ArrayList<CardNotes> clst = Cards.get(i);
            ...
            tableCards[indx] = new CardNotes(value,suit);
            if(cardCount == 0) {
                tableSuite = suit;
            ...
            if(cardCount == 4) {
                ...
                boolean stop = true;
                //Suns Mode
                if(gameModes.contains(new Integer(5))) {
                    connectedNodes.get(winner).AddScore(-15);
                    stop = false;
                }
                //Diamonds Mode
                if(gameModes.contains(new Integer(4))) {
                ...
                //Queens Mode
                if(gameModes.contains(new Integer(3))) {
                    for(int i =0;i<tableCards.length;i++) {
                ...
                //King Heart Mode
                if(gameModes.contains(new Integer(2))){
                    for(int i =0;i<tableCards.length;i++) {
                ...
                if(stop)
                    tricksWon =13;
                // change to 13 after testing
                if(this.tricksWon == 13) {
                    tricksWon = 0;
                    ...
                    // return to 5 after testing dont forget !
                    if(this.tablehead == 4) {
